...
class BaseService(
    ...
    def get_by_id(self, entity_id: int) -> ResponseSchemaType:
        ...
        try:
            ...
            self.logger.error(
                f"Error fetching {self.entity_name} {entity_id}: {str(e)}"
            )
    ...
    def get_all(self, skip: int = 0, limit: int = 100) -> List[ResponseSchemaType]:
        ...
        try:
            ...
            self.logger.debug(
                f"Fetching {self.entity_name}s with skip={skip}, limit={limit}"
            )
    ...
    def get_paginated(
        ...
        try:
            self.logger.debug(
                f"Fetching paginated {self.entity_name}s: page={pagination.page}, limit={pagination.limit}"
            )

            # Get paginated response from repository
            paginated_response = self.repository.get_paginated_response(
                pagination, **filters
            )
            ...
            self.logger.debug(
                f"Retrieved {len(response_items)} {self.entity_name}s (page {pagination.page})"
            )
            return result
    ...
    def create(self, create_data: CreateSchemaType) -> ResponseSchemaType:
        ...
        try:
            ...
            self.logger.info(
                f"{self.entity_name} created successfully with ID: {entity.id}"
            )
            return self.response_schema.model_validate(entity)
    ...
    def update(
        self, entity_id: int, update_data: UpdateSchemaType
    ) -> ResponseSchemaType:
        ...
        try:
            ...
            self.logger.error(
                f"Error updating {self.entity_name} {entity_id}: {str(e)}"
            )
    ...
    def delete(self, entity_id: int) -> ResponseSchemaType:
        ...
        try:
            ...
            if not deleted_entity:
                raise ServiceError(
                    f"Could not delete {self.entity_name} with ID {entity_id}"
                )
            ...
            self.logger.error(
                f"Error deleting {self.entity_name} {entity_id}: {str(e)}"
            )
    ...
    def exists(self, entity_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking if {self.entity_name} {entity_id} exists: {str(e)}"
            )
            raise ServiceError(
